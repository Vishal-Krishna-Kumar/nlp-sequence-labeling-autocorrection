...
class EditModel(object):
  ...
  def insertEdits(self, word):
    ...
    for i in range(1, len(word)+1):
      #The corrupted signal is just preceding character
      corruptLetters = word[i-1]
      #Loop through all alphabets
      for letter in EditModel.ALPHABET:
        #The correct signal is preceding character and the insert character
        correctLetters = "%s%s" % (corruptLetters, letter)

        #The corrected word adds character before i (and lacks the start symbol)
  ...
  def transposeEdits(self, word):
    ...
    for i in range(len(word)-1):
      #The corrupted signal are this character and the character succeding
      corruptLetters = word[i:i+2] 
      #The correct signal is just the transpose of characters
      correctLetters = corruptLetters[::-1]

      #The corrected word is tranpose of character and succeding character
  ...
  def replaceEdits(self, word):
    ...
    for i in range(len(word)):
      ...
      #Loop through all alphabets
      for letter in EditModel.ALPHABET:
        #The correct signal is replacement character
        correctLetters = letter

        #The corrected word is replacement character with the corrputed character
