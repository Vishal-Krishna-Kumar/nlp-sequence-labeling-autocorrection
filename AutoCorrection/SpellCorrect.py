...
class SpellCorrect:
  ...
  def correctSentence(self, sentence):
    ...
    for i in range(1, len(sentence)-1): #ignore <s> and </s>
      #get edits and probs for the current word
      editsAndProbs = self.editModel.editProbabilities(sentence[i])
      #iterate through all edits and check which sentence have high likely score
      for edit, prob in editsAndProbs:
        #form trial sentence from edit
        trialSentence = sentence[:i] + [edit] + sentence[i+1:]
        #get log prob for the trail sentence from lm
        logProb = self.languageModel.score(trialSentence)
        #get total score
        totalScore = logProb + prob #TODO need to check this
        #check total score
